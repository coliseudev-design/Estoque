/**
 * @deprecated ARQUIVO LEGADO — NÃO USAR OU MODIFICAR
 *
 * Este arquivo foi o router monolítico original de sincronização (1.280 linhas).
 * Foi refatorado em 7 módulos com responsabilidade única, localizados em:
 *   src/routes/sync/index.js    → router principal (montagem)
 *   src/routes/sync/catalog.js  → /sellers, /catalog
 *   src/routes/sync/customers.js   → /customers, /new-customer, /confirm-customer/:id
 *   src/routes/sync/financials.js  → /payment-species, /financials
 *   src/routes/sync/orders.js      → /orders
 *   src/routes/sync/natureza.js    → /natureza, /payment-conditions
 *   src/routes/sync/performance.js → /performance, /sales-rankings, /company-logo
 *   src/routes/sync/priceTables.js → /price-tables, /product-prices
 *
 * Este arquivo NÃO é importado por nenhum módulo em produção.
 * Mantido SOMENTE como referência histórica para auditorias.
 * Pode ser removido após validação completa dos módulos em produção.
 *
 * @see src/routes/sync/index.js
 * @module routes/sync.legacy [DEPRECATED]
 */
'use strict';
    ...
    try {
        ...
        try {
            natureza = await fbQuery(
                req.company,
                `SELECT N.ID_NATUREZA AS id, TRIM(N.DESCRICAO) AS descricao,
                        TRIM(N.DESCRICAO_NOTA) AS descricaoNota, N.CODIGO_FISCAL AS codigoFiscal,
                        N.ES AS es, N.MOB_ORDEM AS mobOrdem
