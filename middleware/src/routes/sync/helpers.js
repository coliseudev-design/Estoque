...
const { getFirebirdCreds } = require('../../middleware/auth');
 ...
 */
async function fbQuery(req, sql, params = []) {
    const company = req.company;
    if (!company) {
        throw new Error('Empresa não autenticada na requisição');
    }
    const fbCreds = getFirebirdCreds(req);
    if (!fbCreds || !fbCreds.fb_host) {
        if (process.env.FB_MOCK === 'true') {
            return [];
        }
        logger.warn('[Sync] Credenciais Firebird ausentes ou expiradas para a requisição', { companyId: company.id });
        throw createError('Credenciais do banco Firebird indisponíveis. Por favor, reautentique.', 401, 'FIREBIRD_CREDENTIALS_MISSING');
    }
    const fullCompany = { ...company, ...fbCreds };
    try {
        return await fbManager.queryFor(fullCompany, sql, params);
    } catch (err) {
 ...
 * @returns {Promise<any>}
 */
async function fbExec(req, sql, params = []) {
    const company = req.company;
    if (!company) {
        throw new Error('Empresa não autenticada na requisição');
    }
    ...
    const fullCompany = { ...company, ...fbCreds };
    return fbManager.executeFor(fullCompany, sql, params);
}
...
function makePushHandler(entity, arrayKey) {
    return async function (req, res, next) {
        try {
            ...
            // [FIX] Extrai branchId do JWT/header para isolar dados por filial no Redis
            ...
            await store.upsert(companyId, entity, data, branchId);
            logger.info(`[Sync/${entity}] ${data.length} itens recebidos`, { companyId, branchId });
...
module.exports = { fbQuery, fbExec, makePushHandler, safeInt, validateOrder, padDate, padTime, getFirebirdCreds };
