...
const { fbQuery, makePushHandler, getFirebirdCreds } = require('./helpers');
...
router.get('/sellers', async (req, res, next) => {
    try {
        ...
        // [FIX] Usa branchId do JWT para isolar vendedores por filial
        const branchId = req.branch?.id || null;
        const cached = await store.get(companyId, 'sellers', branchId);
        ...
        try {
            ...
            const fbHost = getFirebirdCreds(req)?.fb_host;
            if (!sellers.length && fbHost) {
                sellers = await fbQuery(req,
        ...
        const fbHost = getFirebirdCreds(req)?.fb_host;
        const source = (sellers.length > 0 && fbHost) ? 'firebird' : 'pending_worker';
...
router.get('/catalog', async (req, res, next) => {
    try {
        const companyId = req.company.id;
        // [FIX] Usa branchId do JWT para isolar catálogo por filial
        const branchId = req.branch?.id || null;
        const cached = await store.get(companyId, 'products', branchId);
        if (cached.data.length > 0) {
        ...
        const products = await fbQuery(req, sql, params);
