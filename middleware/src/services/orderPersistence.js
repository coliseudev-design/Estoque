...
/**
 * Recupera os pedidos que estão em status 'pending' ou 'error'.
 * @returns {Array} Lista de pedidos presos.
 */
function getStuckOrders() {
    ...
    try {
        const rows = database.prepare(`
            SELECT id, status, erp_order_id, error_message, customer_name, seller_name, total_amount, created_at, updated_at
            FROM order_index
        ...
        logger.error('[OrderPersistence] Falha ao listar pendências', { error: err.message });
        return [];
    }
}

/**
 * Remove um pedido do SQLite.
 */
function deleteOrder(id) {
    const database = initDb();
    if (!database) return;
    try {
        database.prepare('DELETE FROM order_index WHERE id = ?').run(id);
    } catch (err) {
        logger.error('[OrderPersistence] Falha ao deletar pedido', { id, error: err.message });
    }
}

/**
 * Redefine o status do pedido para pending no SQLite.
 */
function retryOrder(id) {
    const database = initDb();
    if (!database) return;
    try {
        database.prepare("UPDATE order_index SET status = 'pending', error_message = NULL WHERE id = ?").run(id);
    } catch (err) {
        logger.error('[OrderPersistence] Falha ao redefinir status de retry no pedido', { id, error: err.message });
    }
}

module.exports = { hydrate, persistIndex, persistConfirm, persistError, getStuckOrders, deleteOrder, retryOrder };
