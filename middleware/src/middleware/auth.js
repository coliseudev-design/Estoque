...
async function findCompanyById(companyId) {
    ...
    try {
        ...
        const fbCreds   = company ? {
            ...
            fb_database:  company.fb_database,
            fb_user:      company.fb_user,
            fb_password:  company.fb_password,
            fb_charset:   company.fb_charset,
            fb_wire_crypt: company.fb_wire_crypt,
        } : null;
        _cache.set(cacheKey, { company: safePart, fbCreds, expiresAt: Date.now() + CACHE_TTL });
        return { company: safePart, fbCreds };
    } catch (err) {
...
async function requireApiKey(req, res, next) {
    ...
    // [FIX] Permite que o Worker identifique a filial via header X-Branch-Id
    // O app mobile usa o JWT para isso; o Worker usa este header explícito.
    const headerBranchId = req.headers['x-branch-id'];
    if (headerBranchId) {
        req.branch = { id: headerBranchId };
    }

    logger.debug('[Auth] Autenticado (API-Key)', { companyId: company.id, name: company.name, branchId: headerBranchId || null, path: req.path });
    next();
}
...
async function requireDeviceJwt(req, res, next) {
    ...
    try {
        ...
        let { company, fbCreds } = await findCompanyById(companyId);
