...
// Limites pré-configurados por tipo de rota (dimensionados com base na volumetria de sincronizações)
const syncCatalogLimit = companyRateLimit({ windowMs: 60_000, max: Math.ceil(config.security.rateLimitMax * 2.5) || 500, keyPrefix: 'rl:catalog' });
const syncOrdersLimit = companyRateLimit({ windowMs: 60_000, max: config.security.rateLimitMax || 200, keyPrefix: 'rl:orders' });
const defaultLimit = companyRateLimit({ windowMs: 60_000, max: Math.ceil(config.security.rateLimitMax * 3) || 600, keyPrefix: 'rl:default' });
