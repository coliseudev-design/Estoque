...
pdf_options:
  ...
  headerTemplate: |-
    ...
    <section>
      <span class="left">Coliseu Speed — Documentação Técnica</span>
...
<div style="text-align: center; margin-top: 120px; margin-bottom: 80px;">
  <h1 style="font-size: 36px; color: #1a56db; margin-bottom: 10px;">Coliseu Speed</h1>
...
O **Coliseu Speed** é um ecossistema completo para gestão de vendas externas. Permite que vendedores criem pedidos em campo (mesmo sem internet), sincronizando automaticamente com o ERP (Firebird) da empresa via uma arquitetura cloud intermediária.
...
**Diretório:** `ColiseuSpeed.Configurator/`
...
|----------|--------|---------|
| Mobile APK | 1.0.3 | `ColiseuSpeed_1.0.3.apk` |
| Configurator | 1.0.2 | `ColiseuSpeed_Configurator_1.0.2.exe` |
...
graph LR
    ...
    subgraph "worker/"
        CSP["ColiseuSpeed.Worker.csproj"]
        WS["WorkerService.cs"]
        JOBS["Jobs/"]
        SVCS["Services/"]
    end
    ...
    subgraph "ColiseuSpeed.Configurator/"
        MF["MainForm.cs"]
