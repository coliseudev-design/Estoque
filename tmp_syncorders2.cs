...
/// <summary>
/// SyncOrdersJob ÔÇö Busca pedidos pendentes na VPS e os insere no Firebird.
///
/// Fluxo corrigido (#2):
/// 1. GET /api/orders/pending ÔåÆ lista pedidos que o app Flutter enviou ├á VPS
/// 2. Para cada pedido: transa├º├úo Firebird (cabe├ºalho + itens)
/// 3. Confirma na VPS FORA da transa├º├úo (3 tentativas com backoff)
///    Antes: confirmar dentro da transa├º├úo ÔåÆ HTTP timeout fazia rollback do ERP
///    Agora: Firebird commit independente da VPS; retry na confirma├º├úo
/// 4. Em caso de erro no Firebird: reporta na VPS para o app exibir ao vendedor
///
/// SP MOB_CADASTRAR_PEDIDO ÔÇö Par├ómetros verificados no Firebird (2026-02-23):
...
///
/// SP MOB_CADASTRAR_PEDIDO_ITEM ÔÇö Par├ómetros verificados:
...
///
/// Cad├¬ncia: configur├ível (padr├úo 1 minuto).
...
public sealed class SyncOrdersJob
{
    ...
    /// <summary>
    /// Sem├íforo que garante no m├íximo UMA execu├º├úo simult├ónea do RunAsync.
    /// Evita race condition cr├¡tica no GEN_ID(PEDIDOS,0): se dois ciclos rodassem
    /// em paralelo, ambos leriam o mesmo valor do generator Firebird (n├úo transacional)
    /// e um dos pedidos ficaria com o ID errado.
    /// </summary>
    private readonly SemaphoreSlim _lock = new SemaphoreSlim(1, 1);

    /// N├║mero de tentativas de confirma├º├úo na VPS ap├│s inser├º├úo no ERP (#2).
    ...
    {
        // RACE-GUARD: se ciclo anterior ainda est├í processando pedidos, pula este ciclo.
        ...
        {
            _logger.LogDebug("[OrderSync] Ciclo anterior ainda em execu├º├úo ÔÇö pulando ciclo.");
            return;
        }
        ...
        try
        {
            ...
            try
            {
                ...
                // Pr├®-carrega as filiais (faz cache do ciclo atual)
            ...
            {
                var errMsg = $"ÔÜá Erro ao buscar pedidos na VPS: {ex.Message}";
                _logger.LogError("[OrderSync] {Error}", errMsg);
                _status.AppendLog($"[{DateTime.Now:HH:mm:ss}] {errMsg}");
                return; // finally do bloco externo libera o sem├íforo
            }
            ...
            _status.AppendLog($"[{DateTime.Now:HH:mm:ss}] ­ƒôª {orders.Count} pedido(s) pendente(s) encontrado(s)");
    ...
    private async Task ProcessOrderAsync(PendingOrderDto order, CancellationToken ct)
    {
        ...
        // Valida├º├Áes Cr├¡ticas: se faltar dados, reporta erro IMEDIATAMENTE.
        ...
        if (order.Items == null || order.Items.Count == 0)
            throw new InvalidOperationException($"Pedido rejeitado: n├úo cont├®m itens.");

        // ÔöÇÔöÇ Resolve cliente local ÔåÆ ERP ID ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
        // Se o customerId come├ºa com "local_", o cliente foi cadastrado offline e
        // ainda n├úo recebeu um c├│digo ERP. Buscamos o ID confirmado na VPS.
        // Se ainda pendente, pulamos este pedido ÔÇö ele ser├í reprocessado no pr├│ximo ciclo.
        if (order.CustomerId?.StartsWith("local_", StringComparison.OrdinalIgnoreCase) == true)
        {
            ...
            {
                _logger.LogWarning("[OrderSync] Cliente local {CustomerId} ainda n├úo confirmado no ERP. Pedido {OrderId} aguardando pr├│ximo ciclo.",
                    order.CustomerId, order.Id);
                _status.AppendLog($"[{DateTime.Now:HH:mm:ss}] ÔÅ│ Pedido {order.Id.ToString()[..8]}: cliente ainda pendente. Retry no pr├│ximo ciclo.");
                return; // Mant├®m pedido como pending na VPS ÔÇö n├úo reporta erro
            }
            ...
            // PendingOrderDto ├® um record ÔÇö usamos 'with' para criar c├│pia com ID resolvido
            order = order with { CustomerId = erpCustomerId };
        }

        // FASE 1 ÔÇö Inser├º├úo no Firebird (dentro de uma transa├º├úo at├┤mica)
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError("[OrderSync] Erro Firebird no pedido {OrderId}: {Error}",
                order.Id, errTxt);
            _status.AppendLog(
                $"[{DateTime.Now:HH:mm:ss}] Ô£ù Pedido {order.Id.ToString()[..8]} falhou: {errTxt}");
            ...
            try
            {
                ...
                _status.AppendLog(
                    $"[{DateTime.Now:HH:mm:ss}]   Ôå│ Erro reportado ├á VPS (pedido ficar├í como Erro no app)");
            }
            catch (Exception reportEx)
            {
                _status.AppendLog(
                    $"[{DateTime.Now:HH:mm:ss}]   Ôå│ ÔÜá FALHA ao reportar erro ├á VPS: {reportEx.Message} ÔÇö pedido ficar├í PENDENTE!");
        ...
        // FASE 2 ÔÇö Confirma├º├úo na VPS (FORA da transa├º├úo Firebird)
        // O commit do Firebird j├í ocorreu. Se a VPS falhar aqui, o pedido est├í
        // gravado no ERP. Fazemos retry com backoff exponencial para tolerar
        // instabilidade tempor├íria de rede.
        await ConfirmInVpsWithRetryAsync(order.Id, erpOrderId, ct);
    }

    // ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    // FASE 1 ÔÇö Inser├º├úo at├┤mica no Firebird
    // ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ

    /// <summary>
    /// Executa MOB_CADASTRAR_PEDIDO + MOB_CADASTRAR_PEDIDO_ITEM em uma transa├º├úo.
    ///
    /// Returns: ID_PEDIDO gerado pelo ERP.
    /// Throws: InvalidOperationException se a SP retornar ID inv├ílido.
    /// </summary>
    private async Task<int> InsertIntoFirebirdAsync(PendingOrderDto order, CancellationToken ct)
    {
        ...
        await _firebird.TransactionAsync(async (cmd, innerCt) =>
        {
            ...
            // 1. Inserir cabe├ºalho do pedido via stored procedure
            //    Usa EXECUTE PROCEDURE (raw SQL) + ExecuteScalarAsync para evitar ambiguidade
            //    do CommandType.StoredProcedure com par├ómetros de output no driver Firebird.
            //    Abordagem consistente com SyncAtendenteOrdersJob (linha 107).
            //
            //    SP MOB_CADASTRAR_PEDIDO ÔÇö par├ómetros:
            ...
            // PRAZO_PEDIDO: ID composto da condi├º├úo de pagamento (FORMA_PGTO.ID_FORMA || '_' || ID_ESPECIE)
            ...
            var tipoOperacao = order.NaturezaId ?? string.Empty;
            ...
            // que chegue como UUID ou nulo por bug no app m├│vel.
            if (!int.TryParse(order.SellerId, out var sellerIdInt))
                throw new InvalidOperationException(
                    $"SellerId inv├ílido (n├úo-num├®rico): '{order.SellerId}' para pedido {order.Id}");

            if (!int.TryParse(order.CustomerId, out var customerIdInt))
                throw new InvalidOperationException(
                    $"CustomerId inv├ílido (n├úo-num├®rico): '{order.CustomerId}' para pedido {order.Id}");
            ...
            cmd.Parameters.AddWithValue("@TIPO_OPERACAO", Truncate(tipoOperacao, 20) ?? string.Empty);
            ...
            _logger.LogDebug("[OrderSync] {Diag}", spDiag);  // Debug: diagn├│stico normal da SP
            _status.AppendLog($"[{DateTime.Now:HH:mm:ss}] ÔÜÖ {spDiag}");

            // ExecuteNonQuery: a SP cria o pedido. N├úo capturamos o output via ExecuteScalar
            // porque este driver Firebird .NET n├úo retorna o par├ómetro OUTPUT da SP.
            // GEN_ID(PEDIDOS, 0) l├¬ o valor atual do generator (gerado pela SP).
            // Seguro porque SemaphoreSlim garante UMA execu├º├úo por vez.
            ...
            if (erpOrderId <= 0)
            {
                var errMsg = $"ÔÜá SP retornou ID=0 | {spDiag}";
                _status.AppendLog($"[{DateTime.Now:HH:mm:ss}] {errMsg}");
                throw new InvalidOperationException(
                    $"MOB_CADASTRAR_PEDIDO retornou ID_PEDIDO={erpOrderId} para pedido {order.Id}");
            }

            _status.AppendLog($"[{DateTime.Now:HH:mm:ss}] Ô£ô Pedido {order.Id.ToString()[..8]} integrado. ERP ID={erpOrderId}");
            ...
            //    ao TOTAL_PEDIDO informado, distribu├¡mos o desconto global proporcionalmente.
            ...
            foreach (var (item, itemDiscountValue, itemTotalPrice) in itemsWithDiscount)
            {
                ...
                // BLOCKER fix: TryParse para evitar FormatException se ProductCode chegar inv├ílido
                if (!int.TryParse(item.ProductCode, out var productCodeInt))
                    throw new InvalidOperationException(
                        $"ProductCode inv├ílido: '{item.ProductCode}' no pedido {order.Id}");
                ...
                // e DESCONTO = % do cabe├ºalho PEDIDOS (= 0 porque passamos VALOR_DESCONTO=0 no header).
                // Atualizamos ambos para exibi├º├úo correta na grade do ERP:
                //   VALOR_FINAL_UN = pre├ºo unit├írio l├¡quido (com desconto)
                //   DESCONTO       = % de desconto do item ÔÇö p/ coluna "Valor Desc." na grade
                //
                // Seguran├ºa: como PEDIDOS.VALOR_PEDIDO = totalL├¡quido e PEDIDOS.DESCONTO = 0,
                // o rodap├® "Total Produtos" e "Total Pedido" ambos mostram o net ÔåÆ sem Troco negativo.
                // O SUM_DESC do PEDIDOTOTAL exibe um valor no campo "Desconto (F1)" no rodap├®,
                // mas n├úo afeta Total Produtos nem Total Pedido.
                if (itemDiscountValue > 0 && item.Quantity > 0m)
                {
                    ...
                    // Desconto % calculado sobre o bruto (para exibi├º├úo correta na coluna Valor Desc)
            ...
            //    Aqui: deletamos essa linha e inserimos N parcelas corretas com valor L├ìQUIDO.
            ...
            // SUGGESTION-3 fix: arredonda para 2 casas; ├║ltima parcela absorve o centavo residual
            ...
            if (installments <= 0)
                _logger.LogWarning("[OrderSync] PaymentInstallments={N} inv├ílido para pedido {Id}, usando 1.",
                    order.PaymentInstallments, order.Id);

            _logger.LogInformation(
                "[OrderSync] Parcelamento: {N}x de {Val:F2} (├║ltima={Last:F2}, entrada={E}d, parc={D}d)",
            ...
            for (var i = 1; i <= installments; i++)
            {
                ...
                // ├Ültima parcela absorve o centavo residual (SUGGESTION-3)
            ...
            // Se a SP j├í usou os par├ómetros nativos (hasMultiTenant = true), pulamos esta etapa.
            if (!hasMultiTenant && order.BranchId.HasValue && _cachedBranches != null)
            {
                var branch = _cachedBranches.FirstOrDefault(b => b.Id == order.BranchId.Value);
            }

            // FASE 4: Recalcular totais sumarizados na capa do pedido (ITENS, QTDE_TOTAL, VALOR_CUSTOS)
            // A stored procedure original inseria 0, 0 por padr├úo, o que fazia alguns ERPs filtrarem o pedido
            ...
            // Transa├º├úo commited aqui pelo TransactionAsync ÔÇö ERP j├í tem o pedido
        }, ct);
    ...
    // ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    // FASE 2 ÔÇö Confirma├º├úo na VPS com retry (#2)
    // ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ

    /// <summary>
    /// Confirma o pedido na VPS com backoff exponencial.
    ///
    /// O pedido J├ü EST├ü no Firebird. Esta fase apenas sincroniza o status
    /// na VPS (SQLite de controle) para que o app Flutter receba o ERP ID.
    ///
    /// Tentativas: 1s ÔåÆ 4s ÔåÆ 9s (base┬▓ ├ù 1s).
    /// </summary>
        ...
        string orderId, int erpOrderId, CancellationToken ct)
    {
        for (var attempt = 1; attempt <= VpsConfirmMaxRetries; attempt++)
        {
            ...
            if (attempt < VpsConfirmMaxRetries)
            {
                ...
                _logger.LogDebug("[OrderSync] Aguardando {Delay}s antes da pr├│xima tentativa.", delay.TotalSeconds);
                await Task.Delay(delay, ct);
            }
        }

        _logger.LogError(
            "[OrderSync] Pedido {OrderId} INSERIDO no ERP (ID={ErpId}) mas falha persistente na VPS ap├│s {Max} tentativas. " +
            "O pedido ser├í reprocessado no pr├│ximo ciclo se ainda estiver como 'processing' na VPS.",
    ...
    ///   - Desconto por item (item.Discount em %) ÔÇö aplicado individualmente pelo vendedor
    ///   - Desconto global do pedido (order.DiscountValue ÔêÆ ╬ú descontos por item)
    ///
    /// Regra de neg├│cio (2026-03-25):
    ...
    ///   3. Calcula globalDiscount = order.DiscountValue ÔêÆ ╬ú(descontos individuais)
    ///   4. Identifica candidatos = itens com item.Discount == 0
    ///   5. Distribui globalDiscount proporcionalmente entre os candidatos
    ///      (ou entre todos, se n├úo houver candidatos ÔÇö fallback)
    ///   6. ├Ültimo candidato absorve o centavo residual
    ///
        ...
        ApplyGlobalDiscount(PendingOrderDto order)
    {
        ...
        // 3. Desconto global = total do pedido ÔêÆ descontos individuais j├í embutidos
        //    Clamp em 0 para evitar valor negativo por diferan├ºa de ponto flutuante
        ...
        // 4. Candidatos ao rateio = ├¡ndices dos itens SEM desconto individual
        ...
        // Fallback: se TODOS os itens t├¬m desconto individual, rateia em todos
        ...
        var globalShares    = new decimal[items.Count]; // indexado pelo ├¡ndice do item
    ...
    /// <summary>
    /// Trunca uma string ao tamanho m├íximo aceito pela SP.
    ...
    /// <summary>
    /// Extrai o ID inteiro da condi├º├úo de pagamento.
    ...
    /// Retorna 0 se nulo ou n├úo parse├ível.
    /// </summary>
