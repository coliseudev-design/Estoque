try {
    $asm = [System.Reflection.Assembly]::LoadFrom("C:\Users\rober\.gemini\antigravity\scratch\Coliseu_Sales\worker\bin\Release\net8.0-windows\win-x64\FirebirdSql.Data.FirebirdClient.dll")
    Write-Output "Assembly loaded: $($asm.FullName)"
} catch {
    Write-Error "Failed to load Firebird assembly: $_"
    exit 1
}

$connString = "User=SYSDBA;Password=masterkey;Database=C:\Coliseu\Data\PIVETA.FDB;DataSource=localhost;Port=3050;Dialect=3;Charset=NONE;"
$conn = New-Object FirebirdSql.Data.FirebirdClient.FbConnection($connString)
$conn.Open()
...
while ($reader.Read()) {
    $sellers += [PSCustomObject]@{
        ...
        Nome = $reader["NOME"]
        MobAcesso = $reader["MOB_ACESSO"]
        IdEmpresa = $reader["ID_EMPRESA"]
    }
}
$reader.Close()

Write-Output "--- Active Sellers ---"
$sellers | Format-Table

# 2. Run KPI query for seller 105
# Let's check for each department (1, 2, 3)
Write-Output "--- Running mobMinhasVendasSql for Seller 105 ---"
foreach ($depto in 1, 2, 3) {
    $q = @"
EXECUTE BLOCK (
    VENDEDOR INTEGER = 105,
    MES INTEGER = 5,
    ANO INTEGER = 2026,
    DATA DATE = '2026-05-20',
    EMPRESA INTEGER = $depto
)
RETURNS (
...
"@
    ...
    while ($r2.Read()) {
        Write-Output "Depto $depto - Venda Diaria: $($r2['VENDA_DIARIA']), Venda Mensal: $($r2['VENDA_MENSAL'])"
    }
    $r2.Close()
}
