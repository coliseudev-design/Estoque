...
namespace ColiseuSales.Configurator
{
    public class InstallerManager
    {
        ...
        private readonly string _workerExecutableName = "ColiseuSales.Worker.exe";

        public InstallerManager(string suffix = "")
        {
            if (string.IsNullOrEmpty(suffix))
            {
                _workerServiceName = "ColiseuSales Worker";
                _workerDisplayName = "ColiseuSales Worker";
            }
            else
            {
                _workerServiceName = $"ColiseuSalesWorker_{suffix}";
                _workerDisplayName = $"Coliseu Sales Worker - {suffix}";
            }
        }
        ...
        public string? EnsureWorkerInstalledAndRegistered()
        {
            try
            {
                ...
                var baseExe = Path.Combine(dir, "ColiseuSales.Worker.exe");
        ...
        private bool InstallWindowsService(string binPath)
        {
            ...
            RunSc($"description \"{_workerServiceName}\" \"Serviço de Sincronização do Coliseu Sales ({_workerDisplayName}) (Firebird ↔ VPS)\"");
            return true;
        }
