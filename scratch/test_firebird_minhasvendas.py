...
try:
    ...
    # Parameters: VENDEDOR, MES, ANO, DATA, EMPRESA
    cur.execute(mobMinhasVendasSql, [105, 5, 2026, "2026-05-20", 1])
    cols = [desc[0] for desc in cur.description]
    rows = cur.fetchall()
    print("mobMinhasVendasSql Results:")
    for r in rows:
        print(dict(zip(cols, r)))

    print("\nExecuting mobMinhasVendasRSql...")
    # Parameters: VENDEDOR, DATAI, DATAF, DIA, EMPRESA, DEPTO
    cur.execute(mobMinhasVendasRSql, [105, "2026-05-01", "2026-05-31", "2026-05-20", 1, 1])
    cols_r = [desc[0] for desc in cur.description]
    rows_r = cur.fetchall()
    print("mobMinhasVendasRSql Results:")
    for r in rows_r:
