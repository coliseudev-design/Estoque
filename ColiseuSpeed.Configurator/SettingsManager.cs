...
namespace ColiseuSpeed.Configurator
{
    public class SettingsManager
    {
        ...
        private static string? GetWorkerInstallDir()
        {
            try
            {
                const string svcKey = @"SYSTEM\CurrentControlSet\Services\ColiseuSpeed Worker";
            ...
            bool coliseSpeedEnabled = false, string coliseSpeedBaseUrl = "", string coliseSpeedApiKey = "")
        {
            ...
            // FIX: Parsear a string da UI (ex: "10.10.100.212/7070:COLISEUSPEED") em campos separados
        ...
        /// Suporta formatos:
        ///   "host/port:database"    → (host, port, database)  ex: 10.10.0.212/7070:COLISEUSPEED
        ///   "host:database"         → (host, 3050, database)  ex: servidor:COLISEUSPEED
        ///   "C:\caminho\banco.fdb"  → (localhost, 3050, caminho)
        ///   "COLISEUSPEED"             → (localhost, 3050, COLISEUSPEED)
        /// </summary>
        internal static (string host, int port, string database) ParseFirebirdPath(string path)
        {
            ...
            // Sem ':' — é um alias local (ex: "COLISEUSPEED")
            return ("localhost", 3050, path);
        }
