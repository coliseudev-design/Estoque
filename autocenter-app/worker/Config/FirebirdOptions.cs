...
public sealed class FirebirdOptions
{
    ...
    /// <summary>Habilita criptografia de wire. Enabled por padrão — negocia com o servidor
    /// (funciona com WireCrypt=Required e WireCrypt=Disabled no server).</summary>
    public bool   WireCrypt         { get; set; } = true;

    /// <summary>
    /// Variante da SP MOB_CADASTRAR_PEDIDO no Firebird do cliente.
    /// Standard = 10 params (padrão).
    /// Extended = 11 params — inclui CONDICAO_PAGAMENTO no final
    ///            (ERPs com versão da SP que aceita condição de pagamento separada).
    /// </summary>
    public string SpVariant { get; set; } = "Standard";

    /// <summary>True quando o ERP usa a variante estendida (11 params).</summary>
    public bool UseExtendedSpVariant =>
        string.Equals(SpVariant, "Extended", StringComparison.OrdinalIgnoreCase);
