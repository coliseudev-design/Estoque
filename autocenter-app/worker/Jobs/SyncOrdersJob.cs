...
public sealed class SyncOrdersJob
{
    ...
    private async Task<int> InsertIntoFirebirdAsync(PendingOrderDto order, CancellationToken ct)
    {
        ...
        await _firebird.TransactionAsync(async (cmd, innerCt) =>
        {
            ...
            // Usando ? posicionais como o middleware Direct (provado funcionar com Firebird).
            ...
            // Ordem POSICIONAL — deve corresponder exatamente à assinatura da SP:
            // 1.USUARIO 2.CLIENTE 3.DATA 4.HORA 5.OBSERVACAO 6.PRAZO_PEDIDO
            // 7.TIPO_OPERACAO 8.PAGAMENTO 9.VALOR_DESCONTO 10.TOTAL_PEDIDO [11.CONDICAO_PAGAMENTO]
            cmd.Parameters.Add(new FbParameter { FbDbType = FbDbType.Integer, Value = sellerIdInt });              // 1. USUARIO
            cmd.Parameters.Add(new FbParameter { FbDbType = FbDbType.Integer, Value = customerIdInt });            // 2. CLIENTE
            cmd.Parameters.Add(new FbParameter { FbDbType = FbDbType.VarChar, Value = data });                     // 3. DATA
            cmd.Parameters.Add(new FbParameter { FbDbType = FbDbType.VarChar, Value = hora });                     // 4. HORA
            cmd.Parameters.Add(new FbParameter { FbDbType = FbDbType.VarChar,                                     // 5. OBSERVACAO
                Value = Truncate(order.Notes, 50) ?? string.Empty });
            cmd.Parameters.Add(new FbParameter { FbDbType = FbDbType.VarChar,                                     // 6. PRAZO_PEDIDO
                Value = Truncate(prazoPedido, 20) ?? string.Empty });
            cmd.Parameters.Add(new FbParameter { FbDbType = FbDbType.VarChar,                                     // 7. TIPO_OPERACAO
                Value = Truncate(tipoOperacao, 20) ?? string.Empty });
            cmd.Parameters.Add(new FbParameter { FbDbType = FbDbType.Integer, Value = pagamentoId });              // 8. PAGAMENTO
            // IMPORTANT: passar 0.0 em VALOR_DESCONTO para que PEDIDOS.DESCONTO = 0%.
            // A SP MOB_CADASTRAR_PEDIDO computa DESCONTO = (VALOR_DESCONTO*100)/(TOTAL+VALOR_DESCONTO)
            // e a SP MOB_CADASTRAR_PEDIDO_ITEM faz subquery desse % e aplica em TODOS os itens.
            // Passando 0, PEDIDOS.DESCONTO = 0 e nenhum item recebe desconto automático do cabeçalho.
            // Os descontos individuais são aplicados item a item via GRID-FIX (UPDATE PEDIDO_ITENS).
            // TOTAL_PEDIDO = order.TotalAmount já é o valor líquido (descontos já subtraídos pela store).
            cmd.Parameters.Add(new FbParameter { FbDbType = FbDbType.Double, Value = 0.0 });                          // 9. VALOR_DESCONTO = zero (descontos via GRID-FIX)
            cmd.Parameters.Add(new FbParameter { FbDbType = FbDbType.Double, Value = (double)order.TotalAmount });    // 10. TOTAL_PEDIDO (líquido)

            // Cachear condId evita chamar ParseConditionId duas vezes (code review #5)
            var condId = _fbOpts.UseExtendedSpVariant ? ParseConditionId(order.PaymentConditionId) : 0;
            if (_fbOpts.UseExtendedSpVariant)
            {
                cmd.Parameters.Add(new FbParameter { FbDbType = FbDbType.Integer, Value = condId });              // 11. CONDICAO_PAGAMENTO
            }
            ...
            var spDiag = $"[SP] Pedido {order.Id.ToString()[..8]} | " +
                         ...
                         $"PGTO={pagamentoId}" +
                         (_fbOpts.UseExtendedSpVariant ? $" COND={condId}" : "") +
            ...
            foreach (var (item, itemDiscountValue, itemTotalPrice) in itemsWithDiscount)
            {
                ...
                cmd.Parameters.Add(new FbParameter("PRODUTO",        FbDbType.Integer) { Value = productCodeInt });
                ...
                cmd.Parameters.Add(new FbParameter("VALOR_DESCONTO", FbDbType.Decimal) { Value = itemDiscountValue });
                // Passa o BRUTO para VALOR_TOTAL. A SP mapeia VALOR_FINAL = VALOR_TOTAL.
                // PEDIDOTOTAL usa sum(VALOR_FINAL * DESCONTO/100) → bruto * disc% = desconto correto.
                // Com itens brutos e PEDIDOS.DESCONTO = %, ERP calcula:
                //   Total Produtos = bruto total e Total Pedido = líquido = bruto - desconto. ✅
                var itemGrossTotal = item.UnitPrice * item.Quantity;
                cmd.Parameters.Add(new FbParameter("VALOR_TOTAL",    FbDbType.Decimal) { Value = itemGrossTotal });
