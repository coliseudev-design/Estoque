...
public sealed class VpsApiClient
{
    ...
    public async Task<bool> PushPerformanceAsync(
        ...
        CancellationToken ct = default)
        => await PostAsync("/api/sync/performance", new { performance }, ct);
    ...
    public async Task<bool> PushSalesRankingsAsync(
        ...
        CancellationToken ct = default)
        => await PostAsync("/api/sync/sales-rankings", new { rankings }, ct);
    ...
    /// <summary>
    /// Envia preços por produto × tabela para a VPS (POST /api/sync/product-prices).
    /// Baseado na procedure MOB_TABELAPRECO: PRECO = BASE + (BASE × PRECOT_P%).
    /// </summary>
