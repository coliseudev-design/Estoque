...
public sealed class SyncAtendenteOrdersJob
{
    ...
    private async Task<int> InsertOrderInFirebird(PendingOrderDto order, CancellationToken ct)
    {
        ...
        return await _firebird.ExecuteInTransactionAsync(async conn =>
        {
            ...
            var natureIdVal = (order.NaturezaId == "null" || string.IsNullOrWhiteSpace(order.NaturezaId)) 
                ? null 
                : (order.NaturezaId.Length <= 20 ? order.NaturezaId : order.NaturezaId[..20]);
            cmdHeader.Parameters.AddWithValue("@TIPO_OPERACAO",  (object?)natureIdVal ?? DBNull.Value);
