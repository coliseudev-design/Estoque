...
BEGIN
    ...
    where (natureza_operacao.calc_comissao = 1) and (extract(month from PEDIDOS.data_vencimento) = 5) and (extract(year from PEDIDOS.data_vencimento) = 2026)
    and (PEDIDOS.ID_VENDEDOR = 105) and (PEDIDOS.ID_DEPTO = 1) and (PEDIDOS.TIPO = 1) and (PEDIDOS.STATUS = 2) and (ESTOQUE.STATUS <> 9) and (ESTOQUE.movc = 1)
    ...
    left join natureza_operacao on (natureza_operacao.id_natureza = pedidos.id_natureza)
    where (natureza_operacao.calc_comissao = 1) and ((PEDIDOS.data_vencimento >= '2026-05-20') and (PEDIDOS.data_vencimento <= '2026-05-20'))
    and (PEDIDOS.ID_VENDEDOR = 105) and (PEDIDOS.ID_DEPTO = 1) and (PEDIDOS.TIPO = 1) and (PEDIDOS.STATUS = 2) and (ESTOQUE.STATUS <> 9) and (ESTOQUE.movc = 1)
    into :VEND_D, :COM_D;
    ...
    where (natureza_operacao.calc_comissao = 1) and ((LISTAPEDIDOS_ITENS.tipo_item = 3) or (LISTAPEDIDOS_ITENS.tipo_item = 6)) and (extract(month from LISTAPEDIDOS_ITENS.data_vencimento) = 5) and (extract(year from LISTAPEDIDOS_ITENS.data_vencimento) = 2026)
    and (LISTAPEDIDOS_ITENS.id_tecnico = 105) and (LISTAPEDIDOS_ITENS.ID_DEPTO = 1) and (LISTAPEDIDOS_ITENS.TIPO = 1) and (LISTAPEDIDOS_ITENS.STATUS = 2)
